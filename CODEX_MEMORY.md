...
- 2026-05-11: Google Drive `/file/d/<id>/view` links must be treated as direct document resources during candidate discovery. Do not scan the Drive preview HTML as a generic page; it can produce false candidates such as `Google Docs`, `IE=edge`, `article`, `view`, and fake `.pdf` paths under the same file ID.
- 2026-05-11: If Drive download returns `text/html` with `Google Drive - Can't download file`, the code should report a permission/login/anonymous-download limitation. It is not a valid PDF response; public sharing or future Cookie/browser-auth support is needed.
